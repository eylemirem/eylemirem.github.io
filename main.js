...
$(function() {
  ...
  function set_settings() {
    ...
    settings.defaultredirect = 'https://forms.gle/urnJGJEfPQWcWUuYA';
  ...
  function init_name() {
  	...
  	$('#submit_username').on('click',function() {
  		...
  		if(uname == "") {
  			error = 1;
  			errormsg = 'Lütfen metin girin';
  			uname = "undefined";
  		}
  		if(not_alphanumeric(uname)) {
  			error = 1;
  			errormsg = 'Lütfen sadece harf giriniz ve boşluk bırakmayınız';
  		}  		
    	...
    	$('#submit_avatar').on('click',function() {
    		if($('.selected').length == 1) {
  			...
  			window.avatarexport = /avatar_([^\s]+)/.exec(window.avatar)[1];
    			...
    			alertify.log("Lütfen bir avatar seçiniz","error");
    		}
    	});
  ...
  function init_text() {
  	...
  	$("#description").keyup(function(){
  	  $("#count").text("Kalan karakter: " + (400 - $(this).val().length));
  	});
  	...
  	$('#submit_text').on('click',function() {
  		...
  		if($('#description').val() !== "" && $('#description').val().length < 140) {
  			...
  			error = 1;
  			errormsg = 'Lütfen biraz daha fazla yazın';
			}
  		if($('#description').val().length > 401) {
  		
  			error = 1;
  			errormsg = 'Lütfen biraz daha az yazın';
  		}  		
  		if(error == 0) {
  			$('#text').hide();
    ...
    // When user receives likes
	  $('.userslikes').each(function() {
  		...
  		{ 
  			...
  			themsg = usernames[i] + " postunuzu beğendi ";
